...
io.sockets.on('connection', function(socket){
	...
	socket.on('disconnect',function(data){
		...
		if(typeof player === "undefined"){
			lefts[socket.id] = 0;
			player = dockers[socket.id];
			if(typeof player === "undefined") player = deads[socket.id];
			if(typeof player === "undefined") return;
		} else lefts[socket.id] = 150;
		var text = "~`" + player.color + "~`" + player.name + "~`yellow~` left the game!";
		console.log(text);
		chatAll(text);
	});
	socket.on('pingmsg',function(data){
		if (typeof data === "undefined") return; // || typeof data.time === "undefined") { is unnecessary, server doesn't use it
...
function getPlayer(i){
	var p = "undefined";
	for(var x = 0; x < mapSz; x++) for(var y = 0; y < mapSz; y++) if(p === "undefined") p = players[y][x][i];
	if(typeof p === "undefined") p = dockers[i];
	if(typeof p === "undefined") p = deads[i];
