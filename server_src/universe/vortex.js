module.exports = class Vortex {
  ...
  move() {
    ...
    for (const i in players[this.sy][this.sx]) {
      ...
      if (p.ship == 21 && !this.isWorm) guestMult=0.5*(-1+(22/dist)); //R21 ship gets pushed from a BH if too far, BUT IT'S STILL PULLED WITH FORCE IF TOO CLOSE. Reason this isn't an increment is because someone could get a GUEST at level 21, buy the ship, and then the old *=0.5 would actually be more OP than the old code.
