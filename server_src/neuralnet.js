let mutate = function () { // Low change of high variability, high chance of low.
	return Math.tan(Math.random() * Math.PI * 2) / 100;
}
let activate = function (x) { // Softsign activation function. See wikipedia.
...
module.exports = class NeuralNet {
	...
	save(k) {
		let source = 'server/neuralnets/' + k + '.bot';
	...
	load() {
		...
		let parentCount = Math.floor(Math.random() * 3 + 1);
		for (let p = 0; p < parentCount; p++) {
			let source = 'server/neuralnets/' + Math.floor(Math.random() * neuralFiles) + '.bot';
			if (fs.existsSync(source)) {
				let fileData = fs.readFileSync(source, "utf8").split('\n');
