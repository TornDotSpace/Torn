...
var botFrequency = .8;//higher: more bots. Standard: 1.6
...
var NeuralNet = function(){
	...
	self.save = function(){
		var source = 'server/neuralnets/' + Math.floor(self.type*100+Math.random()*100) + '.txt';
...
var Player = function(i){
	...
	self.testSectorChange = function(){
		...
		if(self.x > sectorWidth){
			self.x = 1;
			self.sx++;
			if(self.sx >= mapSz || self.name === "GUEST" || self.isNNBot){
				giveBounce = true;
				self.sx--;
			...
			self.y = 1;
			self.sy++;
			if(self.sy >= mapSz || self.name === "GUEST" || self.isNNBot){
				giveBounce = true;
				self.sy--;
			...
			self.sx--;
			if(self.sx < 0 || self.name === "GUEST" || self.isNNBot){
				giveBounce = true;
				self.sx++;
				self.x = 5;
				self.driftAngle = self.angle = 3.1415 - self.angle;
			...
			self.sy--;
			if(self.sy < 0 || self.name === "GUEST" || self.isNNBot){
				giveBounce = true;
				self.sy++;
				self.y = 5;
				self.driftAngle = self.angle = -self.angle;
	...
	self.nnBotPlay = function(){
		...
		if(self.isNNBot)
			for(var i in PLAYER_LIST){
				var a = PLAYER_LIST[i];
				if(a.sx == self.sx && a.sy == self.sy)
					if(square(a.x - self.x) + square(a.y - self.y) < square(320)){
						self.net.save();
						self.die();
					}
			}
		...
		for(var a in ASTEROID_LIST){
			var ast = ASTEROID_LIST[a];
			if(ast.sx != self.sx || ast.sy != self.sy) continue;
			var dist2 = hypot2(ast.x, self.x, ast.y, self.y);
			if(dist2 < closeE){target = ast;closeE = dist2;}
		}
	...
	self.checkPlanetCollision = function(){
		...
		if(tick % 25 == 0 && hypot2(p.x,self.x,p.y,self.y) < sectorWidth/2 * 4){
			...
			if(self.name === "GUEST") SOCKET_LIST[self.id].emit("chat",{msg:'You must create an account in the base before you can claim planets!', color:'yellow'});
	...
	self.die = function(b){
		...
		else if(Math.random() < .004 && self.name !== 'GUEST') PACKAGE_LIST[r] = Package(self, r, 2);
		else if(self.name !== 'GUEST') PACKAGE_LIST[r] = Package(self, r, 1);
		else if(self.name !== 'GUEST') PACKAGE_LIST[r] = Package(self, r, 1);//ammo pckg
	...
	self.move = function(){
		...
		for(var i in PLAYER_LIST){
			...
			if(self.sx == p.sx && self.sy == p.sy){
				...
				var guestMult = p.name === "GUEST" ? -1 : 1; 
...
function mute(msg){
	...
	for(var p in PLAYER_LIST){
		...
		if(player.name === name){
			...
			chatAll("~`red~`" + player.name + " has been " + (time > 0?"muted for " + time + " minutes!" : "unmuted!"));
			return;
		}	
	}for(var p in DOCKED_LIST){
		var player = DOCKED_LIST[p];
...
function spawnNNBot(sx,sy,col){
	...
	bot.sy = sy;
	var rand = .33 + 3.67*Math.random();
	bot.experience = Math.floor(Math.pow(2,Math.pow(2,rand)))/8 + 3*rand;//TODO change 8 to 4
	bot.updateRank();
	bot.ship = bot.rank;
	bot.x = bot.y = sectorWidth/2;
	bot.color = col;
