	...
	for (var y = 0; y < mapSz; y++) for (var x = 0; x < mapSz; x++) {
		...
		if (tick % 5 == 0) {
			for (var i in packs[y][x]) {
				...
				// Check for creation 
				if (pack === undefined) {
					pack = packs[y][x][i] = { x: boon.x, y: boon.y, type: boon.type };
		...
		for (var i in beams[y][x]) {
			...
			// Check for deletion
			if (beam.time == 0 || beams[y][x][i] === undefined) {
		...
		for (var i in blasts[y][x]) {
			...
			// Check for deletion
			if (blast.time == 0 || blasts[y][x][i] === undefined) {
				// Send delete
		...
		for (var j in missiles[y][x]) {
			...
			// Check for deletion
			if (missiles[y][x][j] === undefined) {
				sendAllSector('missile_delete', j, x, y);

				delete missilePack[y][x][j];
				continue;
			}
