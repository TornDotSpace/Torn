...
function onCrash(err) {
  ...
  if (Config.getValue("debug", true)) {
    ...
    send_rpc("/crash/", crashReport).finally(setTimeout(function() {
      process.exit(3);
    }, 4000));
  }
}
...
function updateQuests() {
  for (const teamColor in baseMap) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const whatTeam = (Math.random()<.5)?colorSelect(teamColor, "blue", "green", "red"):colorSelect(teamColor, "green", "red", "blue");
...
function updateHeatmap() {
  ...
  for (let i = 0; i < mapSz; i++) {
    for (let j = 0; j < mapSz; j++) {
      ...
      const length = Math.sqrt(r*r+g*g+b*b)+.01;
      r/=length;
      b/=length;
      g/=length;
