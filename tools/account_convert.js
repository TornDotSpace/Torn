...
const MONGO_CONNECTION_STR = `mongodb://localhost:27017/torn`;
const Mongo = require(`mongodb`).MongoClient(MONGO_CONNECTION_STR, { useUnifiedTopology: true });
const PLAYERS_DIR = `../server/players`;
...
function loadPlayerData (playerName, passwordHash) {
    ...
    const readSource = `${PLAYERS_DIR}/${playerName}[${passwordHash}.txt`;
    ...
    const fileData = fs.readFileSync(readSource, `utf8`).split(`:`);
...
function main () {
    Mongo.connect((err, client) => {
        ...
        const player_db = db.collection(`players`);

        fs.readdirSync(PLAYERS_DIR).forEach((file) => {
            const stat = fs.statSync(`${PLAYERS_DIR}/${file}`);

            if (!stat.isDirectory()) {
                console.log(`Beginning conversion of: ${file}`);

                const f_str = file.split(`[`);

                const player_name = f_str[0];
                const player_hash = parseInt(f_str[1].split(`.txt`)[0]);

                console.log(`Got player (${player_name},${player_hash})`);

                const player = loadPlayerData(player_name, player_hash);
