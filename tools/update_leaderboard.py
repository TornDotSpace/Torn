...
def updateLB(conn_str, path):
    ...
    for player in players.find().sort("experience", pymongo.DESCENDING):
        ...
        kills = player['kills']
        rank = player['rank']
        tech = int(((player['thrust2'] + player['radar2'] + player['capacity2'] +
                     player['agility2'] + player['maxHealth2'] + player['energy2'])/6-1)*8*100)/100
        xp = int(player['experience'])
        money = player['money']
        color = "pink" if (player['color'] == 'red') else (
            "lime" if (player['color'] == 'green') else 'cyan')
        ...
        moneyStr = f'{int(money //1000000+.5)}M' if money > 10000000 else f'{int(money//1000+.5)}K'
